...
{
    private const string ServerName = "kusto-mcp";
    ...
    public static void PrintSetupInstructions()
    {
        ...
        Console.WriteLine("kusto-mcp setup instructions");
        Console.WriteLine("============================");
        Console.WriteLine();
        Console.WriteLine("Run 'kusto-mcp install' to automatically add the server to your");
...
""");
    ...
    private static ServerCommand ResolveServerCommand()
    {
        ...
        // If running as a published native binary, point directly at it
        ...
        // Otherwise, use `dotnet run` pointing at the project
        return new ServerCommand("dotnet", ["run", "--project", FindProjectPath()]);
    }

    private static string FindProjectPath()
    {
        // Walk up from the base directory to find the .csproj
        ...
        if (baseDir is not null)
        {
            ...
            {
                var csproj = Path.Combine(dir.FullName, "Kusto.Cli.Mcp.csproj");
